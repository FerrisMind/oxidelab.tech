// Hero section functionality - Updated: 2025-01-20 15:45
console.log(
  "Hero.js loaded - New version with GitHub downloads and fixed download flag"
);
const Hero = {
  heroSection: null,
  downloadBtn: null,
  ...
  async handleDownloadClick(item) {
    ...
    try {
      // Determine which type of file to download based on the text
      let fileType = "";
      if (text.includes("CPU") && !text.includes("GPU")) {
        // CPU only version
        fileType = "CPU";
      } else if (text.includes("CPU") && text.includes("GPU")) {
        // CPU + GPU version
        fileType = "CPU+GPU";
      }

      if (fileType) {
        const fileUrl = await this.findFileByType(fileType);
        console.log("Found fileUrl:", fileUrl);

        if (fileUrl) {
          this.downloadFile(fileUrl);
          const fileName = fileUrl.split("/").pop();
          this.showDownloadMessage(`Скачивание ${fileName}...`);
        } else {
          console.log("File not found, showing message");
          this.showDownloadMessage(`Файл с припиской ${fileType} не найден`);
        }
      } else {
      ...
      this.showDownloadMessage("Ошибка при поиске файла");
    } finally {
      // Always reset the downloading flag
      this.isDownloading = false;
      console.log("Download process completed, flag reset");
    }

    this.closeDownloadDropdown();
  },

  // Find file by type (CPU or CPU+GPU)
  async findFileByType(fileType) {
    try {
      console.log("findFileByType called with:", fileType);

      // Version variable - change this to update all file versions
      const version = "0.10.23";

      // Base URL for GitHub releases
      const baseUrl = `https://github.com/FerrisMind/Oxide-Lab/releases/download/${version}`;

      // Generate file URL based on type
      let fileName;
      if (fileType === "CPU") {
        fileName = `Oxide-Lab_${version}_x64-CPU.exe`;
      } else if (fileType === "CPU+GPU") {
        fileName = `Oxide-Lab_${version}_x64-GPU+CPU.exe`;
      } else {
